...
printf '{"active":true,"filename":"%s","percent":0,"elapsed":"00:00:00","eta":"--:--:--","speed":"..."}' \
...
while ! grep -q "^progress=end" "$PROGRESS_PIPE_FILE" 2>/dev/null; do # Search in the file, a line that starts with that text, the -q is to make a quiet grep.
    ...
    if [ -n "$OUT_TIME_MS" ] && [ "$OUT_TIME_MS" -gt 0 ] 2>/dev/null; then # Check if the variable is not null and if is grater than 0.
        ...
        printf '{"active":true,"filename":"%s","percent":%d,"elapsed":"%s","eta":"%s","speed":"%s"}' \
            "$FILENAME" "$PERCENT" "$ELAPSED" "$ETA" "${SPEED:-...}" \
            > "$STATUS_FILE"
    fi
...
printf '{"active":true,"filename":"%s","percent":100,"elapsed":"","eta":"00:00:00","speed":"done"}' \
    "$FILENAME" > "$STATUS_FILE"
sleep 4
