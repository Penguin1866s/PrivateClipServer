...
QUEUE_JSON="${3:-[]}"   # third arg: JSON array of pending filenames, default empty
...
# Write initial status immediately so the widget appears right away
# ETA --> Estimated Time of Arrival.
printf '{"active":true,"filename":"%s","percent":0,"elapsed":"00:00:00","eta":"--:--:--","speed":"...","queue":%s}' \
    "$FILENAME" "$QUEUE_JSON" > "$STATUS_FILE"

# Poll every 2 seconds until ffmpeg finishes (= PROGRESS_PIPE_FILE contains "progress=end")
while ! grep -q "^progress=end" "$PROGRESS_PIPE_FILE" 2>/dev/null; do # Search in the file, a line that starts with that text, the -q is to make a quiet grep.
    ...
    if [ -n "$OUT_TIME_MS" ] && [ "$OUT_TIME_MS" -gt 0 ] 2>/dev/null; then # Check if the variable is not null and if is grater than 0.
        ...
        if [ "$DURATION" -gt 0 ] 2>/dev/null; then
            ...
            # Check that SPEED_INT contains numbers and is greater than 0.
            # because some times, the speed is 0, because of some reason ffmepg say that speed is 0 or N/A, and if we do the division with 0, the script will crash, and the fprintf will never be executed.
            if [[ "$SPEED_INT" =~ ^[0-9]+$ ]] && [ "$SPEED_INT" -gt 0 ] 2>/dev/null; then
        ...
        [ -z "$CURRENT_QUEUE" ] && CURRENT_QUEUE="[]"

        # The "-..." is to replace with that default value("...") if the variable of SPEED is void.
        printf '{"active":true,"filename":"%s","percent":%d,"elapsed":"%s","eta":"%s","speed":"%s","queue":%s}' \
...
printf '{"active":true,"filename":"%s","percent":100,"elapsed":"","eta":"00:00:00","speed":"done","queue":[]}' \
